...
final class Version20260403010000 extends AbstractMigration
{
    ...
    {
        $this->addSql("SET @has_status := (SELECT COUNT(*) FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'locations' AND COLUMN_NAME = 'status');");
        $this->addSql("SET @add_status_sql := IF(@has_status = 0, 'ALTER TABLE locations ADD status VARCHAR(32) NOT NULL DEFAULT ''ACTIVE''', 'SELECT 1');");
        $this->addSql('PREPARE stmt FROM @add_status_sql');
        ...
        $this->addSql("SET @has_fk := (SELECT COUNT(*) FROM information_schema.TABLE_CONSTRAINTS WHERE CONSTRAINT_SCHEMA = DATABASE() AND CONSTRAINT_NAME = 'FK_APPOINTMENTS_SLOT');");
        $this->addSql("SET @add_fk_sql := IF(@has_fk = 0, 'ALTER TABLE appointments ADD CONSTRAINT FK_APPOINTMENTS_SLOT FOREIGN KEY (slot_id) REFERENCES appointment_slots (id) ON DELETE SET NULL', 'SELECT 1');");
        $this->addSql('PREPARE stmt FROM @add_fk_sql');
        $this->addSql('EXECUTE stmt');
        $this->addSql('DEALLOCATE PREPARE stmt');

        $this->addSql("SET @has_idx := (SELECT COUNT(*) FROM information_schema.STATISTICS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'appointments' AND INDEX_NAME = 'IDX_APPOINTMENTS_SLOT');");
        $this->addSql("SET @add_idx_sql := IF(@has_idx = 0, 'CREATE INDEX IDX_APPOINTMENTS_SLOT ON appointments (slot_id)', 'SELECT 1');");
        $this->addSql('PREPARE stmt FROM @add_idx_sql');
        $this->addSql('EXECUTE stmt');
        $this->addSql('DEALLOCATE PREPARE stmt');
    }
